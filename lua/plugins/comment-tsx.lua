...
return {
    ...
    opts = function(_, _)
        local function add_comment()
            ...
            for i = line1 - 1, 1, -1 do
                ...
                if l == "{/*" then
                    ...
                    break
                elseif l == "*/}" then
                    break -- found nested end first
                end
            end

            -- search downward for "*/}"
            local finish = nil
            for i = line2 + 1, vim.fn.line("$") do
                local l = vim.trim(vim.fn.getline(i))
                if l == "*/}" then
                    finish = i
                    break
                elseif l == "{/*" then
                    break -- found nested start first
                end
            end

            if start and finish and start < line1 and finish > line2 then
                -- inside existing JSX comment block: remove the markers
        ...
        -- Override keybinding only for TSX
        vim.api.nvim_create_autocmd("FileType", {
            pattern = { "typescriptreact" },
            callback = function()
                vim.keymap.set("v", "gc", add_comment, { buffer = true, desc = "Toggle JSX {/* */} comment" })
                vim.keymap.set("n", "gc", remove_comment, { buffer = true, desc = "Toggle JSX {/* */} comment" })
            end,
        })
    end,
}
